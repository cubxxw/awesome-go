...
<h2 id="_1-何为内存" tabindex="-1"><a class="header-anchor" href="#_1-何为内存" aria-hidden="true">#</a> 1 何为内存</h2>
<p>说到内存，及时没有任何的软件基础知识，那么第一印象应该想到的是如下实物，如图 1 所示。</p>
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/zlvD7RteFS.png!large" alt="awsef"></p>
<h3 id="图-1-物理内存条" tabindex="-1"><a class="header-anchor" href="#图-1-物理内存条" aria-hidden="true">#</a> 图 1 物理内存条</h3>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/1dphL5SMT9.png!large" alt="awsef"></p>
<h3 id="图-2-计算机存储媒介金字塔模型" tabindex="-1"><a class="header-anchor" href="#图-2-计算机存储媒介金字塔模型" aria-hidden="true">#</a> 图 2 计算机存储媒介金字塔模型</h3>
...
<h3 id="表-1-硬盘与内存对比表" tabindex="-1"><a class="header-anchor" href="#表-1-硬盘与内存对比表" aria-hidden="true">#</a> 表 1 硬盘与内存对比表</h3>
<p><img src="http://sm.nsddd.top/sm202303031006244.png" alt="image-20230303100620097"></p>
<p>所以将大部分程序逻辑临时用的数据，全部都存在内存之中，比如，变量、全局变量、函数跳转地址、静态库、执行代码、临时开辟的内存结构体（对象）等。</p>
<h2 id="_2-内存为什么需要管理" tabindex="-1"><a class="header-anchor" href="#_2-内存为什么需要管理" aria-hidden="true">#</a> 2 内存为什么需要管理</h2>
...
<h3 id="表-2-自动与非自动内存管理的语言" tabindex="-1"><a class="header-anchor" href="#表-2-自动与非自动内存管理的语言" aria-hidden="true">#</a> 表 2 自动与非自动内存管理的语言</h3>
...
<h2 id="_3-操作系统是如何管理内存的" tabindex="-1"><a class="header-anchor" href="#_3-操作系统是如何管理内存的" aria-hidden="true">#</a> 3 操作系统是如何管理内存的</h2>
<p>计算机对于内存真正的载体是物理内存条，这个是实打实的物理硬件容量，所以在操作系统中定义这部门的容量叫物理内存。</p>
<p>实则物理内存的布局实际上就是一个内存大数组，如图 3 所示。</p>
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/G72Sqei8Um.png!large" alt="awsef"></p>
<h3 id="图-3-物理内存布局" tabindex="-1"><a class="header-anchor" href="#图-3-物理内存布局" aria-hidden="true">#</a> 图 3 物理内存布局</h3>
...
<p>一个操作系统是不可能只运行一个程序的，那么这个大数组物理内存势必要被 N 个程序分成 N 分，供每个程序使用。但是程序是活的，一个程序可能一会需要 1MB 的内存，一会又需要 1GB 的内存。操作系统只能取这个程序允许的最大内存极限来分配内存给这个进程，但这样会导致每个进程都会多要去一大部分内存，而这些多要的内存却大概率不会被使用，如图 4 所示。<img src="https://cdn.learnku.com/uploads/images/202205/23/58489/IQZQ8Vfysj.png!large" alt="awsef"></p>
<h3 id="图-4-物理内存分配的困局" tabindex="-1"><a class="header-anchor" href="#图-4-物理内存分配的困局" aria-hidden="true">#</a> 图 4 物理内存分配的困局</h3>
<p>当 N 个程序同时使用同一块内存时，那么产生读写的冲突也在所难免。这样就会导致这些昂贵的物理内存条，几乎跑不了几个程序，内存的利用率也就提高不上来。</p>
<p>所以就引出了操作系统的内存管理方式，操作系统提供了虚拟内存来解决这件事。</p>
<h3 id="_3-1-虚拟内存" tabindex="-1"><a class="header-anchor" href="#_3-1-虚拟内存" aria-hidden="true">#</a> 3.1 虚拟内存</h3>
<p>所谓虚拟，类似是假、凭空而造的大致意思。对比上图 3.3 所示的物理内存布局，虚拟内存的大致表现方式如图 5 所示。</p>
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/wPo2lh1X0C.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/sF5qe0wzlq.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/bEPjaY1g3V.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/F11MJBbpMd.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/Pi18r4mFiz.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/xznDie9ouV.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/DwgpY8A3UC.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/JnCscKIxBw.png!large" alt="awsef"></p>
...
<div class="language-text ext-text line-numbers-mode"><pre v-pre class="language-text"><code>func Loop(nums [awsef]int, step int) {
   l := len(nums)
   for j := 0; j &lt; l; j += 1 {
       nums[j] = 4 //访问内存，并写入值
   }
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>其中 C.CBytes () 方法可以将 Golang 的 [awsef] byte 切片转换成 unsafe.Pointer 类型。利用这个转换功能，来分析一下是如何封装 memcpy () 函数的：</p>
<div class="language-text ext-text line-numbers-mode"><pre v-pre class="language-text"><code>func Memcpy(dest unsafe.Pointer, src [awsef]byte, length int) {
   srcData := C.CBytes(src)
   C.memcpy(dest, srcData, C.size_t(length))
}

</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>新封装的 Memcpy () 的第一个形参是任意指针类型，表示拷贝的目标地址，第二个形参是 [awsef] byte 类型，表示被拷贝的源数据，第三个参数表示本次拷贝数据的长度。因为 C 语言中的 memcpy () 函数原型如下：</p>
<div class="language-text ext-text line-numbers-mode"><pre v-pre class="language-text"><code>#include &lt;string.h>

void *memcpy(void *dst, const void *src, size_t n);

</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>对于 src 数据源形参需要 [awsef] byte 转换为 unsafe.Pointer，因此在调用 C 的接口是通过 C.CBytes () 转换了一下。</p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/4FVN22kCa6.png!large" alt="awsef"></p>
...
<p>Buf 被初始化之后，需要给 Buf 赋予让调用方传入数据的接口，这里允许一个 Buf 的内存可以赋予 [awsef] byte 类型的源数据，方法名称是 SetBytes ()，定义如下：</p>
<div class="language-text ext-text line-numbers-mode"><pre v-pre class="language-text"><code>//zmem/mem/buf.go
...
<p>（1）将 [awsef] byte 源数据 src 通过 C 接口的内存拷贝，给 Buf 的 data 赋值。这里要注意的是被拷贝的 data 的起始地址是 b.head。</p>
<p>（2）拷贝之后 Buf 的有效数据长度要相应的累加偏移，具体的过程如图 14 所示。</p>
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/7Sm6uBF3yK.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/uKA54pZVZ6.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/2ZKbGrxHAn.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/wzJf7M0K3b.png!large" alt="awsef"></p>
...
<p>Zbuf 的 Read () 方法是将数据填充到 Zbuf 的 Buf 中。Read () 方法是将被填充的数据作为形参 [awsef] byte 传递进来。</p>
<div class="language-text ext-text line-numbers-mode"><pre v-pre class="language-text"><code>//zmem/mem/zbuf.go
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/RhCSKk0Jl2.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/DWdNm4x8Ig.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/eWrHJLTEdA.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/3rqWEnJnzb.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/nN1qHH89RQ.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/ivmjaMfAeU.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/TxIGJR26fZ.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/V85EFusAJ5.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/1dkEKE2ak9.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/H6WUHzoNme.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/sFSOXbjSsK.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/gIEVG5Pnof.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/X4ZvnfDz7e.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/uDZB3Hy50v.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/2fxXkOn6an.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/CDvFCKTFqv.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/z1KuvVWFyd.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/Eoa2O9XiiT.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/kbkO9zhs1K.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/qK281tfKAX.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/6NQvk0q25r.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/rrio7zjjY4.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/VmLPHYEJAb.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/uGkWn9hFmu.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/MMIyWhR3Bi.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/tGQtrTmWNl.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/Wul3ss2Vdd.png!large" alt="awsef"></p>
...
<p><img src="https://cdn.learnku.com/uploads/images/202205/23/58489/v0iUFYrCxc.png!large" alt="awsef"></p>
