...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>如果想方便的话，<strong>可以把 redis 的路径加到系统的环境变量里</strong>，这样就省得再输路径了，后面的那个 redis.windows.conf 可以省略，如果省略，会启用默认的。输入之后，会显示如下界面：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p><img src="@source/markdown/images/nYFityhS3rGOm8d.jpg" alt="Redis 安装"></p>
...
<p>Redis 的配置文件位于 Redis 安装目录下，文件名为 <strong><code v-pre>redis.conf</code></strong>(Windows 名为 <code v-pre>redis.windows.conf</code>)。</p>
<p>你可以通过 <strong><code v-pre>CONFIG</code></strong> 命令查看或设置配置项。</p>
<hr>
...
<td style="text-align:left"><code v-pre>save &lt;seconds&gt; &lt;changes&gt;</code>Redis 默认配置文件中提供了三个条件：**<code v-pre>save 900 1save 300 10save 60 10000</code>**分别表示 900 秒（15 分钟）内有 1 个更改，300 秒（5 分钟）内有 10 个更改以及 60 秒内有 10000 个更改。</td>
...
<td style="text-align:left">设置 Redis 连接密码，如果配置了连接密码，客户端在连接 Redis 时需要通过 AUTH <code v-pre>&lt;password&gt;</code> 命令提供密码，默认关闭</td>
</tr>
<tr>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h4 id="实例-3" tabindex="-1"><a class="header-anchor" href="#实例-3" aria-hidden="true">#</a> 实例</h4>
<div class="language-text ext-text line-numbers-mode"><pre v-pre class="language-text"><code>redis 127.0.0.1:6379> DEL runoob
