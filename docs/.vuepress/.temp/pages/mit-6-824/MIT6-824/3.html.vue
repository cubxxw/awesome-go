...
<h1 id="第3节-sac" tabindex="-1"><a class="header-anchor" href="#第3节-sac" aria-hidden="true">#</a> 第3节 SAC</h1>
<br>
...
<hr>
<nav class="table-of-contents"><ul><li><router-link to="#可扩展性-scalability">可扩展性（Scalability）</router-link></li><li><router-link to="#可用性-availability">可用性（Availability）</router-link></li><li><router-link to="#一致性-consistency">一致性（Consistency）</router-link></li><li><router-link to="#end-链接">END 链接</router-link></li></ul></nav>
<p>[TOC]</p>
<h2 id="可扩展性-scalability" tabindex="-1"><a class="header-anchor" href="#可扩展性-scalability" aria-hidden="true">#</a> 可扩展性（Scalability）</h2>
...
<p><img src="http://sm.nsddd.top/smimage-20221118224720719.png" alt="image-20221118224720719"></p>
...
<p><img src="http://sm.nsddd.top/smimage-20221118224731489.png" alt="image-20221118224731489"></p>
...
<p><img src="http://sm.nsddd.top/smimage-20221118225308937.png" alt="image-20221118225308937"></p>
...
<p><img src="http://sm.nsddd.top/smimage-20221118224742401.png" alt="image-20221118224742401"></p>
...
<h2 id="可用性-availability" tabindex="-1"><a class="header-anchor" href="#可用性-availability" aria-hidden="true">#</a> 可用性（Availability）</h2>
<p>另一个重要的话题是容错。</p>
...
<p><img src="https://906337931-files.gitbook.io/~/files/v0/b/gitbook-legacy-files/o/assets%2F-MAkokVMtbC7djI1pgSw%2F-MDCEhMz5hoV7rz1neAf%2F-MDCEj-P9bkRrJuDYO9Y%2Fimage.png?alt=media&amp;token=25c74d0e-bf22-4ca1-8886-a3b084b87b12" alt="img"></p>
...
<p><img src="https://906337931-files.gitbook.io/~/files/v0/b/gitbook-legacy-files/o/assets%2F-MAkokVMtbC7djI1pgSw%2F-MDCEhMz5hoV7rz1neAf%2F-MDCJSnq90CNK2C9NWBh%2Fimage.png?alt=media&amp;token=c7f3250f-46e6-48ff-b0c9-d26a2071aec3" alt="img"></p>
...
<p>一个是非易失存储（non-volatile storage，类似于硬盘）。这样当出现类似电源故障，甚至整个机房的电源都故障时，我们可以使用非易失存储，比如硬盘，闪存，SSD之类的。我们可以存放一些checkpoint或者系统状态的log在这些存储中，这样当备用电源恢复或者某人修好了电力供给，我们还是可以从硬盘中读出系统最新的状态，并从那个状态继续运行。所以，这里的一个工具是非易失存储。因为更新非易失存储是代价很高的操作，所以相应的出现了很多非易失存储的管理工具。同时构建一个高性能，容错的系统，聪明的做法是避免频繁的写入非易失存储。在过去，甚至对于今天的一个3GHZ的处理器，写入一个非易失存储意味着移动磁盘臂并等待磁碟旋转，这两个过程都非常缓慢。有了闪存会好很多，但是为了获取好的性能，仍然需要许多思考。</p>
<ul>
<li></li>
</ul>
<p>对于容错的另一个重要工具是复制（replication），不过，管理复制的多副本系统会有些棘手。任何一个多副本系统中，都会有一个关键的问题，比如说，我们有两台服务器，它们本来应该是有着相同的系统状态，现在的关键问题在于，这两个副本总是会意外的偏离同步的状态，而不再互为副本。对于任何一种使用复制实现容错的系统，我们都面临这个问题。lab2和lab3都是通过管理多副本来实现容错的系统，你将会看到这里究竟有多复杂。</p>
<h2 id="一致性-consistency" tabindex="-1"><a class="header-anchor" href="#一致性-consistency" aria-hidden="true">#</a> 一致性（Consistency）</h2>
...
<p><img src="https://906337931-files.gitbook.io/~/files/v0/b/gitbook-legacy-files/o/assets%2F-MAkokVMtbC7djI1pgSw%2F-MDHdgjhRDEZbEp7faY1%2F-MDHhviIzWotOfHQ5gDj%2Fimage.png?alt=media&amp;token=13ab3478-d3f2-4955-a643-2cca17f3d052" alt="img"></p>
...
<p><img src="https://906337931-files.gitbook.io/~/files/v0/b/gitbook-legacy-files/o/assets%2F-MAkokVMtbC7djI1pgSw%2F-MDHdgjhRDEZbEp7faY1%2F-MDHxMhHG1Ugjsbdhaq4%2Fimage.png?alt=media&amp;token=66b58375-13e2-469d-9a4a-22c0c3992c7b" alt="img"></p>
<p>现在某个客户端发送了一个put请求，并希望将key 1改成值21。这里或许是KV服务里面的一个计数器。这个put请求发送给了第一台服务器，</p>
<p><img src="https://906337931-files.gitbook.io/~/files/v0/b/gitbook-legacy-files/o/assets%2F-MAkokVMtbC7djI1pgSw%2F-MDHdgjhRDEZbEp7faY1%2F-MDHxnhxCCRW5NHcG_mG%2Fimage.png?alt=media&amp;token=0764ab2e-5758-4dc5-b6f9-62683ecb0596" alt="img"></p>
...
<p><img src="https://906337931-files.gitbook.io/~/files/v0/b/gitbook-legacy-files/o/assets%2F-MAkokVMtbC7djI1pgSw%2F-MDHdgjhRDEZbEp7faY1%2F-MDHxz0v7oXVofDALMmv%2Fimage.png?alt=media&amp;token=635b772a-eaf9-4b18-a499-7768ae6f37dd" alt="img"></p>
