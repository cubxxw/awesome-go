...
<p>当前项目的<strong>可执行文件</strong>。<code v-pre>cmd</code> 目录下的每一个<strong>子目录名称都应该匹配可执行文件</strong>。比如果我们的项目是一个 <code v-pre>grpc</code> 服务，在 /cmd/<strong>myapp</strong>/main.go 中就包含了启动服务进程的代码，编译后生成的可执行文件就是 <strong>myapp</strong>。</p>
...
<p>/pkg 在许多开源项目中都使用了，但未被普遍接受，并且 Go 社区中的某些人不推荐这样做。</p>
...
<p>IaaS，PaaS，系统和容器编排部署配置和模板（docker-compose，kubernetes/helm，mesos，terraform，bosh）。请注意，在某些存储库中（尤其是使用 kubernetes 部署的应用程序），该目录的名字是 <strong>/deploy</strong>。</p>
